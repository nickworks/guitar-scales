...
impl FretboardApp {
    ...
    fn draw_fret_markers(&self, painter:Painter, num_cols:usize, x:f32, y:f32){
        for fret in 0..num_cols {
            self.draw_fret_marker(fret, painter.to_owned(), Pos2 {
                x: x + (fret as f32 * self.settings.space_fret),
                y: y
            });
        }
    }
